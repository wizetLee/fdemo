            ...
            Builder(builder: (context) {
              return GestureDetector(
                  onTap: () {
                    ...
                    // RepaintBoundary -
                    // RenderPhysicalModel
                    var f = Scaffold.of(context)?.context.findRenderObject() as RenderBox;

                    // 找父节点中的某个renderObject
                    // Scaffold.of(context)?.context.findAncestorRenderObjectOfType()

                    RepaintBoundary? c = Scaffold.of(context)?.context.findAncestorWidgetOfExactType();
                    var ccc = Scaffold.of(context)?.context.findAncestorRenderObjectOfType();
                    if (c != null) {
                      print("xxxx");
                    }
                    // 找到对应的renderObject
                    ...
                    var result = [1, 2, 3, 4, 5, 6, 7, 8].map((e) => e).take(100);
  ...
  Widget popupBtn() {
    return PopupMenuButton<String>(
      itemBuilder: (context) {
        ...
        var i = PopupMenuItem<String>(child: Text("xxx"), onTap: () {
